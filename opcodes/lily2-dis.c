...
#if DEBUG
  printf ("lily2_extract: %c %x ", param_ch, param_pos);
#endif
  opc_pos = 32;
...
print_insn (bfd_vma memaddr, struct disassemble_info *info)
{
  ...
  struct lily2_opcode const * opcode;
  ...
  for (opcode = &lily2_opcodes[0];
       opcode < &lily2_opcodes[lily2_num_opcodes];
       ++opcode)
    {
          ...
          for (s = opcode->args; *s != '\0'; ++s)
            {
              switch (*s)
                ...
                case 'r':
                  lily2_print_register (*++s, opcode->encoding, insn, info);
                  break;

                case 'X':
                  addr = lily2_extract ('X', opcode->encoding, insn) << 2;
                ...
                default:
                  if (strchr (opcode->encoding, *s))
                    lily2_print_immediate (*s, opcode->encoding, insn, info);
                  else
